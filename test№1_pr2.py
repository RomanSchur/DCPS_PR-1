#* тест оновлення показників вже існуючого лічильника
import unittest
import copy
...
class test1_pr2(unittest.TestCase):
   def test(self):
        ...
        new_data = {"ID": 1, "Дата": "02.03.2025", "Денне_споживання(кВт)": 45.0, "Нічне_споживання(кВт)": 15.0, "Загальна сума оплати на теперішню дату": 0}
        Calculate_price(new_data)
        updated_data = next(i for i in Information_storage if i["ID"] == 1)
        self.assertEqual(updated_data["Денне_споживання(кВт)"], 45.0)
        self.assertEqual(updated_data["Нічне_споживання(кВт)"], 15.0)
        self.assertEqual(updated_data["Загальна сума оплати на теперішню дату"], 1512)
