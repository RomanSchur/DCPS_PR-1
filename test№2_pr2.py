#* тест отримання показників від нового лічильника
import unittest
import copy
...
class test1_pr2(unittest.TestCase):
   def test(self):
        ...
        new_data = {"ID": 2, "Дата": "02.03.2025", "Денне_споживання(кВт)": 50.0, "Нічне_споживання(кВт)": 40.0, "Загальна сума оплати на теперішню дату": 0}
        Calculate_price(new_data)
        updated_data = next(i for i in Information_storage if i["ID"] == 2)
        self.assertEqual(updated_data["Денне_споживання(кВт)"], 50.0)
        self.assertEqual(updated_data["Нічне_споживання(кВт)"], 40.0)
        self.assertEqual(updated_data["Загальна сума оплати на теперішню дату"], 2184)
